...
"""
"""
yDF = pd.read_csv(f"dataset/y_train.csv")
ya = yDF.loc[yDF["Id"].isin(brfss["Id"])]
print(ya.shape)
ya.to_csv("dataset/ya.csv", sep=",", index=False)"""
# ************************************************************************************************
...
x_test = np.genfromtxt("dataset/modified_xtest.csv", delimiter=",", skip_header=1)
ids = np.genfromtxt("test_ids.csv", delimiter=",")


xxx = pd.DataFrame(x_train)
print(xxx)
...
create_csv_submission(ids, prediction, "NEW_y_predRF.csv")  # F1: 0.226     | Acc: 0.81
"""
